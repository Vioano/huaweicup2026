...
\begin{document}
\hypersetup{pdfsubject={Paper review v9}}
\maketitle
\begin{center}\small 审阅稿 v9\end{center}
\begin{abstract}
...
\end{lstlisting}
\textbf{源码对应：} \texttt{834d8c9}，\texttt{\seqsplit{src/q1/branch\_aid.py::construct}}（第 359--438 行）。上述局部算法聚焦单个波次内的分支选择与分配，\texttt{pieces} 为跨波次累积更新的映射字典。前置条件假定算子包单汇点合法性检查与同高度无跨核计算边已由外围断言验证（不满足时外围抛出 \texttt{\seqsplit{UnsupportedStructure}}）；容量上限 \texttt{\seqsplit{max(320,\ n\_tasks)}} 仅为构造器内部工程规模限制而非官方系统硬上限；外围波次遍历（\texttt{\seqsplit{\_wave\_domain}}）、调度全序重组与异常处理见完整源码。
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{减少搬运量反而增加完成时间的反例（C04）}\label{tab:05-p2-2}\normalsize
\begin{tabular}{@{}
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{启用和关闭共享缓存的配对实验}\label{tab:06-p3-2}\normalsize
\begin{tabular}{@{}
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{附加比值选择条件的反例与消融（R9F）}\label{tab:06-p3-3}\normalsize
\begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{三问主口径加速比综合对比}\label{tab:07-experiments-1}\normalsize
\begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{求解效率与在线评测调用统计}\label{tab:07-experiments-2}\normalsize
\begin{tabular}{@{}
...
针对多核调度需求，本文算法协同优化算子划分、核心分配与执行顺序三个决策维度：
...
方案完成时间由官方评测程序推进的离散事件仿真时钟周期（cycles）度量，求解效率由主机运行墙钟秒数度量；本文所呈现的数据反映在规定基准与配置下的模拟测试记录，不代表在真实 NPU 芯片上的板级实测结果。实验表明，通过图拓扑与硬件特征构造少量结构化方案，结合理论必要条件实施剪枝保护，并利用受限的在线评测预算进行比较择优，能够在赛题建议的高效时间范围内获得符合官方目标的调度解。
...
\bibitem{ref20} BACCELLI F, COHEN G, OLSDER G J, QUADRAT J P，Synchronization and Linearity: An Algebra for Discrete Event Systems，Chichester：John Wiley \& Sons，1992。
