"""Frozen, bounded 500-cell P1 unified E0-900 benchmark; no retries or historical plan reuse."""
...
HERE = 'src/q1_benchmarks/s59ee_v4_full500_e0900.py'
MANIFEST = 'src/q1_benchmarks/s59ee_v4_full500_e0900_manifest.json'
RESULTS = ROOT / 'output/p1-v4-full500-e0900-s59ee'
CELLS = [(f'{i:03d}', k) for i in range(1,101) for k in range(1,6)]


def available_bytes():
    snapshot = subprocess.check_output(['vm_stat'], text=True)
    size = int(re.search(r'page size of (\d+) bytes', snapshot).group(1))
    values = dict(re.findall(r'^(Pages (?:free|inactive|speculative|purgeable)):\s+([\d.]+)', snapshot, re.M))
...
def check():
    ...
    assert manifest['maximum_calls'] == {'solver':500,'external_E0':500,'internal_E1':2400,'E2':0,'retries':0}
    assert manifest['timeouts_seconds'] == {'solver':300,'external_E0':900,'batch':4500}
    assert manifest['workers_max'] == 4 and manifest['worker_ramp'] == [1,2,4]
    assert manifest['per_cell_e1_limit'] == {'k1':0,'k2_to_k5':6}
...
def run(run_id):
    ...
    meta=dict(run_id=run_id,source_commit=SOURCE,runner_commit=head,manifest=spec,
              ...
              completed=0,stop_reason=None,worker_ramp=[1,2,4],
              available_bytes_at_start=available_bytes(),
              cold_start='fresh interpreter per cell; OS caches not flushed')
    h.write(batch/'batch.json',meta)
    stop=threading.Event()
    results={}
    with tempfile.TemporaryDirectory(prefix='p1-v4-full500-input-') as tmp:
        inputs=Path(tmp)
        ...
        def worker(job):
            ...
            try:
                ...
                if row['calls']['internal_E1']>(0 if k==1 else 6): raise RuntimeError('per-cell E1 bound exceeded')
                if info['stop_reason']=='first-score-failure': raise RuntimeError('internal E1 score failure; stop full batch')
                if stop.is_set():
                ...
                row['evaluation']=smoke.process([sys.executable,'-B',h.OFFICIAL/'code/multicore_cut_evaluate_problem_1.py',graph,plan,
                    ...
                    '--log-output',folder/'official.log'],folder,'e0',900,deadline,lambda:row['calls'].__setitem__('external_E0',1))
        ...
        with ThreadPoolExecutor(max_workers=4) as pool:
            while next_index<len(CELLS) or inflight:
                limit=1 if successful<4 else 2 if successful<20 else 4
                free=available_bytes()
                limit=min(limit, 4 if free>=8*1024**3 else 2 if free>=5*1024**3 else 1 if free>=4*1024**3 else 0)
                if limit==0 and not inflight:
                    stop.set();meta['stop_reason']='available_memory_below_4GiB';break
        ...
        meta['status']='complete' if successful==500 else 'stopped'
