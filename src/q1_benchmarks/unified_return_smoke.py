"""Frozen three-cell return smoke controller; source binding and EXECUTE are separate gates.
...
RESULTS = ROOT / 'results/a/q1-unified-return-smoke-20260925'
CELLS = ['008', '084', '095']
GIB4 = 4 * 1024**3
...
def ledger(stdout, diagnostics=None):
    ...
    if not 0 <= len(returns) <= len(starts) <= 5:
...
def verify(spec, spec_path):
    ...
    if spec['cells'] != [[case,5] for case in CELLS] or spec['timeouts_seconds'] != {'solver':300,'external_E0':180,'batch':1200} or spec['maximum_calls'] != {'solver':3,'external_E0':3,'internal_E1':15,'E2':0,'retries':0}:
    ...
    dependencies = [(p,source) for p in paths] + [(p,head) for p in (HERE,'src/q1_benchmarks/bounded_full4_e0.py',str(spec_path.relative_to(ROOT)))] + [(p,source) for p in ('uv.lock','pyproject.toml','docs/a/source-manifest.json','AI chats/P1多Pipe链构造证明/附件/r1-p1_s6607/p1_phase_cut.py')]
...
def run(spec_path, run_id, authorization):
    ...
    started = time.perf_counter(); deadline = started+1200
    ...
    failure=None
    try:
        with tempfile.TemporaryDirectory(prefix='q1-unified-return-smoke-') as temp:
            for folder,row in rows:
                ...
                try:
                    ...
                    verify(spec,spec_path)
                    row['evaluation']=process([sys.executable,'-B',h.OFFICIAL/'code/multicore_cut_evaluate_problem_1.py',graph,plan,'--config',h.OFFICIAL/'data/config.txt','--output',result,'--trace-output',folder/'trace.json','--log-output',folder/'official.log'],folder,'e0',180,deadline,lambda:launched('external_E0'))
                    ...
                    if 'online_ledger' not in row and stdout.exists():
                        ...
                        except Exception as exc: row['online_ledger']={'actual_e1_calls':None,'error':str(exc),'confirmed_worker_calls_lower_bound':0,'worker_calls_upper_bound':5}
