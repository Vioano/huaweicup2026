"""Export the completed four-cell unified smoke without scoring or rerunning."""
...
RUN = ROOT / 'results/a/q1-unified-smoke-20260925/20260924T1800Z-unified4-root'
...
def main():
    ...
    assert batch['calls']=={'solver':4,'external_E0':4,'internal_E1_confirmed':9,'internal_E1_upper_bound':9,'E2':0,'retries':0}
    assert not OUT.exists()
    OUT.mkdir()
    records=[]; receipt=[]
    for case in ('051','044','031','008'):
        ...
        for item in safe_run['artifacts'].values(): item['path']=item['path'].removeprefix('results/a/q1-unified-smoke-20260925/20260924T1800Z-unified4-root/')
        ...
        missing={'provenance.environment.threads':'Batch did not measure runtime thread count','provenance.measurement.seed':'Deterministic runner; no seed recorded','provenance.measurement.offline_costs':'Offline preparation cost not quantified by this smoke batch'}
        record={
          ...
          'parameters':{'selected':run['selected'],'candidate_limit':4,'internal_E1_timeout_seconds':60,'solver_timeout_seconds':300,'external_E0_timeout_seconds':120,'workers':1,'retries':0,'stop_reason':diag['stop_reason']},
          ...
          'provenance':{'producer_session':'nikolastarx/s-6607cb2735304751b36662035723372b','task_url':'https://github.com/huaweibei123/huaweicup2026/issues/33',
            'solver':{'source':source(batch['source_commit'],'src/q1/unified.py','main'),'authors':['nikolastarx'],'method':'Structural features select and E1-rank up to four direct P1 plans; final external E0 checks selected plan.','references':[],'upstream':[],'selected_algorithm_id':run['selected'],'selected_solver_commit':batch['source_commit']},
            'runner':{'source':source(batch['runner_commit'],'src/q1_benchmarks/unified_smoke.py','main'),'argv':['<local-python>','-B','src/q1_benchmarks/unified_smoke.py','run',batch['run_id']],'working_directory':'.'},
            'environment':{**batch['environment'],'peak_rss_bytes':run['solver']['sampled_peak_group_rss_bytes']},
            'measurement':{'started_at':run['started_at'],'finished_at':run['finished_at'],'seed':None,'repeat_index':0,'cold_start':False,
              ...
              'budget':{'wall_seconds':300,'candidate_limit':4,'stop_reason':diag['stop_reason']},
              'calls':{'solver':1,'E0':1,'E1':run['online_ledger']['actual_e1_calls'],'E2':0},'offline_costs':None,'failure':None},'missing_reasons':missing},
          'notes':['Only four of 100 official P1 graphs at k5; preview, no full-matrix attainment claim.',
                   'All E1 winner and external E0 Makespan/movement fields matched exactly.',
        ...
        receipt.append({'case':case,'makespan':result['makespan'],'selected':run['selected'],'solver_wall_seconds':run['solver']['wall_seconds'],'external_E0_wall_seconds':run['evaluation']['wall_seconds'],'internal_E1_calls':run['online_ledger']['actual_e1_calls'],'baseline':baseline_result['makespan'],'original_run_sha256':digest((cell/'run.json').read_bytes()),'original_result_sha256':digest((cell/'result.json').read_bytes()),'original_trace_sha256':digest((cell/'trace.json').read_bytes())})
    feed=OUT/'board-feed-20260924T1800Z-unified4.json';dump(feed,{'schema_version':1,'submission_version':1,'records':records})
