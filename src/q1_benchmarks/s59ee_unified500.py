...
HERE = 'src/q1_benchmarks/s59ee_unified500.py'
MANIFEST = 'src/q1_benchmarks/s59ee_unified500_manifest.json'
RESULTS = ROOT / 'output/p1-unified500-s59ee'
...
def check():
    ...
    assert manifest['maximum_calls'] == {'solver':500,'external_E0':500,'internal_E1':1600,'E2':0,'retries':0}
    assert manifest['timeouts_seconds'] == {'solver':300,'external_E0':180,'batch':7200}
...
def run(run_id):
    ...
    meta=dict(run_id=run_id,source_commit=SOURCE,runner_commit=head,manifest=spec,
              ...
              completed=0,stop_reason=None,worker_ramp=[2,4,8],cold_start='fresh interpreter per cell; OS caches not flushed')
    h.write(batch/'batch.json',meta)
    stop=threading.Event()
    results={}
    with tempfile.TemporaryDirectory(prefix='p1-unified500-input-') as tmp:
        inputs=Path(tmp)
        ...
        def worker(job):
            ...
            try:
                ...
                row['online_ledger']=smoke.ledger((folder/'solver.stdout.txt').read_text(),info)
                ...
                if row['calls']['internal_E1']>4: raise RuntimeError('per-cell E1 bound exceeded')
                if stop.is_set():
        ...
        with ThreadPoolExecutor(max_workers=8) as pool:
            while next_index<len(CELLS) or inflight:
                limit=2 if successful<4 else 4 if successful<20 else 8
