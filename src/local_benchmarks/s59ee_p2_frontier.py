...
def cell(case, cores, source, batch, deadline, stopped, limits):
    ...
    record = {"case_id": case, "problem": "P2", "cores": cores, "algorithm_id": "q2-adaptive-frontier",
    ...
    try:
        if stopped.is_set() or time.monotonic() >= cell_deadline:
        ...
        solver = [sys.executable, "-B", "-m", "src.q2_nikolastarx.adaptive_frontier", graph,
                  "--config", OFFICIAL / "data/config.txt", "--cores", str(cores),
                  "--output", plan, "--evidence", online, "--wall", str(limits["solver"])]
        record["calls"]["solver"] = 1
        record["solver"] = run_child(solver, source, min(limits["solver"], max(1, cell_deadline - time.monotonic())), paths, source)
        ...
        if (online / "adaptive_frontier/plan.json").read_bytes() != plan.read_bytes():
        ...
        record["solver_evidence"] = {"status": ledger["status"], "calls": ledger["calls"],
                                     "route": ledger["attempts"][0]["detail"].get("selected_strategy") or ledger["attempts"][0]["detail"].get("adaptive_route"),
        ...
        record["artifacts"]["online_plan"] = {"path": (online / "adaptive_frontier/plan.json").relative_to(ROOT).as_posix(),
                                                "sha256": sha(online / "adaptive_frontier/plan.json")}
        ...
        record["calls"]["E0"] = 1
        record["evaluation"] = run_child(official, ROOT, min(limits["E0"], max(1, cell_deadline - time.monotonic())), paths, source)
...
def main():
    ...
    if subprocess.check_output(["git", "show", f"{runner_commit}:src/local_benchmarks/s59ee_p2_frontier.py"], cwd=ROOT) != Path(__file__).read_bytes():
        raise RuntimeError("runner bytes differ from fixed commit")
    if subprocess.check_output(["git", "show", f"{runner_commit}:src/local_benchmarks/s59ee_p2_frontier_manifest.json"], cwd=ROOT) != MANIFEST.read_bytes():
    ...
    meta = {"schema": declaration["schema"], "batch_id": batch.name,
            ...
            "runner_path": "src/local_benchmarks/s59ee_p2_frontier.py",
    ...
    results = []
    phases = ((jobs[:1], 1), (jobs[1:8], 2), (jobs[8:], declaration["max_workers"]))
