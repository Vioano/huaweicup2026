"""Guarded R9 layered query affinity; static singleton construction only.

Adapted from Pro message 6ea4bc79-c48b-4b9a-9eac-90b1d4aad4a8,
R9_layered_affinity_r9.py SHA-256
2e6221b4e618d2007aa451db7466d6d985e6347135154f38f6cb46d47501f131.
No official Task, Step, or evaluator is called by this module.
"""
...
from .construct import UnsupportedStructure, validate_graph
from evaluation_validation import EvaluationValidationError

PIPES = ('PIPE_M', 'PIPE_V')
COPY = {'COPY_IN', 'COPY_OUT'}

class GuardError(UnsupportedStructure):
    pass
...
class RawIndex:
    ...
    def build(cls, graph):
        try:
            ...
            raise GuardError(f'invalid original graph: {exc}') from exc
...
def construct_layered(graph, cores, *, capacity, cross_delay_cycles):
    """Return a two-field singleton plan and static evidence, or raise GuardError.

    This pure constructor does not verify its own static certificate with the
    official Task/Step/evaluator. Callers must keep that acceptance separate.
    """
    need(type(cores) is int and cores > 0, 'cores must be a positive integer')
    need(type(cross_delay_cycles) is int and cross_delay_cycles >= 0,
         'cross delay must be a nonnegative integer')
    need(isinstance(capacity, dict) and set(capacity) == {'L1', 'UB'}
         and all(type(v) is int and v > 0 for v in capacity.values()),
         'capacity requires positive L1 and UB byte limits')
    delay = cross_delay_cycles
    index=RawIndex.build(graph); ports=_ports(index)
