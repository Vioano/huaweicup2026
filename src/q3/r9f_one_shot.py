"""Admitted one-shot R9F 068/K5 diagnostic. This measures diagnostics, not solver time."""
...
FIXED_PLAN = ROOT / 'AI chats/20260925-Pro-P3-多层查询亲和/附件/R9F_plan_068_K5.json'
PLAN_SHA256 = 'ee8364e769437cb226cbe95cde845c0bbea59b2fd4e84dc47806d517949b323b'
...
def old_control(path, sha256, *, graph_sha256, config_sha256, official_code_sha256):
    ...
    if control.get('schema') != 'layered-control-v1' or control.get('case_id') != '068' or control.get('cores') != 5:
        raise ValueError('old control schema/case/cores differ')
    for label, path_key, hash_key in (
            ('source feed', 'source_feed_path', 'source_feed_sha256'),
            ('pair manifest', 'pair_manifest_path', 'pair_manifest_sha256')):
        ref = ROOT / control.get(path_key, '')
        if (not ref.resolve().is_relative_to(ROOT)
                or not re.fullmatch(r'[0-9a-f]{64}', control.get(hash_key, ''))
                or not ref.is_file() or digest(ref) != control[hash_key]):
            raise ValueError(f'old {label} reference differs')
...
def decide(candidate_m3, candidate_m2, old):
    """P2 is accepted only with nonworse M2 and exact nonworse M2/M3."""
    ...
    return {'run_p2': candidate_m3 < old['M3'],
            'accepted': (candidate_m3 < old['M3'] and candidate_m2 <= old['M2']
                         ...
                         >= Fraction(old['M2'], old['M3']))}


def worker(args):
    plan_path = within(args.candidate, strict=True) / 'case_068_multicore_res.json'
    if args.plan_sha256 != PLAN_SHA256 or digest(FIXED_PLAN) != PLAN_SHA256 or digest(plan_path) != PLAN_SHA256:
        raise ValueError('R9F fixed plan byte identity differs')
    official, _ = verify_source(args.source, {'068'})
    phase = args.worker
    ...
    receipt = {'status': 'running', 'phase': phase, 'counts_started': counts,
               'source_commit': args.source, 'runner_source_sha256': args.source_sha256,
    ...
    def observe(frame, event, value):
        ...
        if event == 'return' and phase == 'p3':
            ...
            if name == 'Task3':
                ...
                try:
                    receipt['guard'] = check_layered_prepared(graph, plan, captured, layers=6)
    ...
    try:
        ...
        if counts != caps or (phase == 'p3' and receipt.get('guard', {}).get('status') != 'passed'):
...
def run_phase(args, phase, deadline, report):
    ...
    cmd = [sys.executable, '-B', '-m', 'src.q3.r9f_one_shot', str(args.candidate),
           ...
           '--source-sha256', args.source_sha256, '--worker', phase]
    ...
    write(args.output / 'run.json', report)
    if proc.returncode:
        raise RuntimeError(f'{phase} worker failed; slot consumed, no retry')
...
def main():
    ...
    p.add_argument('candidate', type=Path, help='existing directory containing the exact R9F plan bytes')
    p.add_argument('output', type=Path)
    p.add_argument('--source', required=True)
    p.add_argument('--source-sha256', required=True,
                   help='SHA-256 of this committed runner source')
    ...
    if a.plan_sha256 != PLAN_SHA256 or digest(FIXED_PLAN) != PLAN_SHA256:
        raise ValueError('R9F fixed plan SHA-256 differs')
    if not re.fullmatch(r'[0-9a-f]{64}', a.source_sha256) or digest(Path(__file__)) != a.source_sha256:
        raise ValueError('runner source SHA-256 differs')
    plan_path = a.candidate / 'case_068_multicore_res.json'
    if digest(plan_path) != PLAN_SHA256:
        raise ValueError('R9F fixed plan byte identity differs')
    official, inputs = verify_source(a.source, {'068'})
    if a.worker:
        worker(a)
        return
    ...
    report = {'status': 'running', 'started_at': utc(), 'source_commit': a.source,
              'plan_sha256': a.plan_sha256, 'runner_source_sha256': a.source_sha256,
              'admission_file': str(admission_path), 'admission_sha256': a.admission_sha256,
              'old_control': old, 'fixed_plan_source': str(FIXED_PLAN),
    ...
    try:
        ...
        report.update(finished_at=utc(), diagnostic_wall_seconds=time.monotonic() - cold_started)
        write(a.output / 'run.json', report)
