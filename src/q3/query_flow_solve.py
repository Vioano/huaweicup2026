"""One guarded R8 proposal after a fresh forest incumbent, with paired P2 gate.
...
from . import forest_solve, query_flow
...
def evaluate_candidates(index, cores, evaluate_p3, evaluate_p2, save,
    ...
    """
    construct = query_flow.construct if construct is None else construct
    ...
    policy = {"strategy": "query_flow_affinity_r8", "p3_calls": p3_calls,
              "p2_calls": 0, "acceptance": "M3 strict, M2 nonworse, G nonworse"}
    selection = {**selection, "query_flow_policy": policy}
    record = {"name": "query_flow_affinity_r8", "status": "unstarted"}
    try:
        proposal, meta = construct(index, cores, cross_delay=cross_delay,
                                   capacity=capacity)
    ...
    record["p3"] = {"makespan": new_p3["makespan"],
                    "artifacts": save("p3", "query_flow", proposal, new_p3) or {}}
    ...
    p2_calls = 0
    for label, plan in (("incumbent", old_plan), ("query_flow", proposal)):
        p2_calls += 1
    ...
    m20, m21 = (record[key + "_p2"]["makespan"] for key in ("incumbent", "query_flow"))
    ...
    if record["m2_nonworse"] and record["g_nonworse"]:
        winner = proposal, new_p3, meta["strategy"]
...
def main():
    ...
    def save_unscored(plan):
        plan_bytes = encoded(plan)
        path = args.evidence / "query-flow-proposal.json"
