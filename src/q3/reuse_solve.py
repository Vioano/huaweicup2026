"""Try Cartesian input reuse only within the remaining three-call budget."""
import hashlib
...
def evaluate_candidates(index, cores, evaluate, save):
    ...
    if calls >= 3:
        policy.update(status="skip", skip_reason="three_call_budget_already_used",
                      strategy="forest_reuse_grid")
        return winner, calls, records, {**selection, "reuse_policy": policy}
    ...
    record = {"name": "forest_reuse_grid", "strategy": metadata["strategy"],
              "metadata": metadata, "makespan": None}
    selection = {**selection, "reuse_policy": policy}
    ...
    try:
        ...
        return winner, calls, records + [record], selection
    artifacts = save("forest_reuse_grid", proposal, result) or {}
