...
function mirrorNotice(){
  ...
  if(runtime?.mode!=='central_mirror'){banner.hidden=true;return;}
  ...
  banner.textContent='中央已核成绩 · 本机只读镜像。原件已由中央核对，本机未重新核验。快照 '+new Date(runtime.generated_at).toLocaleString('zh-CN')+'（本地时间）'+
  ...
  $('#polling-note').textContent='页面每 5 秒读取本机已接收的中央快照；独立同步程序负责传输。快照保留全部历史、筛选和来源；原件通过固定 Git 链接按需读取。中央已核不代表本机逐原件复核，也不代表重新运行评估器。';
}
