...
let batchRequest=0,batchRows=[],batchResult=null,detailRequest=0,focusChoice='initial';
if(focusProblem){
  ...
  $('#batch-title').textContent=focusProblem+' · 固定算法批次';
  document.title=focusProblem+' · 方案成绩台';
  if(focusProblem==='P3'){
    $('#metric option[value="cache_gain"]').textContent='P3 Cache 加速比 · × ↑';
    $('#metric option[value="cache_hit_rate"]').textContent='P3 字节命中率 · % ↑';
  }
...
try{
  ...
  if(saved?.version===1&&typeof saved.savedAt==='number'&&Date.now()-saved.savedAt<3600000){
    ...
    if(focusProblem)focusChoice=saved.run||saved.algorithm?'batch':'history';
    const restoredMetric=focusProblem==='P3'&&['cache_gain','cache_hit_rate'].includes(saved.cacheMode)?saved.cacheMode:saved.metric;
    if([...$('#metric').options].some(o=>o.value===restoredMetric)&&(!['cache_gain','cache_hit_rate'].includes(restoredMetric)||focusProblem==='P3')){
      mainMetric=restoredMetric;$('#metric').value=mainMetric;
    }
...
async function checkVersion(){
  ...
  try{
    ...
    sessionStorage.setItem(restoreKey,JSON.stringify({version:1,savedAt:Date.now(),metric:mainMetric,
      ...
      scrolls:[...document.querySelectorAll('.board-scroll')].map(x=>({top:x.scrollTop,left:x.scrollLeft})),detailScroll:$('#detail').scrollTop,
...
function options(id,values){const el=$(id),value=pendingRestore?(pendingRestore[id==='#algorithm'?'algorithm':'run']||''):el.value;el.innerHTML='<option value="">'+(id==='#algorithm'?'探索：历史逐格最佳':'所有已接收批次')+'</option>'+values.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');el.value=values.includes(value)?value:'';}
...
function renderBatchList(restoreScroll){
  ...
  const rows=(top?batchResult.full:batchRows).filter(r=>[r.run_id,...r.algorithm_ids,...r.solver_commits].join(' ').toLowerCase().includes(search));
...
async function loadBatches(restoreScroll){
  ...
  if(!cases.length){
    batchResult={batches:[],full:[],full_complete_count:0,complete:[],complete_count:0};batchRows=[];renderBatchList();
  ...
  try{
    ...
    batchResult=result;batchRows=result.batches;
    if(focusChoice==='initial'&&result.full.length){
      focusChoice='batch';
      const run=result.full[0].run_id;
      if(![...$('#run').options].some(o=>o.value===run))$('#run').add(new Option(run,run));
      $('#run').value=run;
    }
    renderBatchList(restoreScroll);
...
$('#history-view').onclick=()=>{focusChoice='history';$('#algorithm').value='';$('#run').value='';$('#reported').checked=false;renderBatchList();refresh(true);};
...
function render(){if(!data)return;const metric=$('#metric').value;const values=data.cells.filter(c=>c.best?.eligible&&(metric!=='cache_gain'||c.best.cache_pair_verified)).map(c=>c.best.metrics[metric]).filter(x=>Number.isFinite(x));const low=values.length?Math.min(...values):null,high=values.length?Math.max(...values):null;
...
function openCache(mode='cache_gain'){
  ...
  dialog.innerHTML=`<div class="cache-heading"><div><h2 id="cache-title">P3 · ${hits?'Cache 字节命中率':'Cache 配对对照'}</h2><p>当前算法、批次及算例筛选 · ${rows.length}/${cells.length} 格可展示</p></div><button id="cache-close" autofocus>返回成绩表 ×</button></div><div class="cache-tabs"><button id="cache-pairs" aria-pressed="${!hits}">配对加速对照 · ${paired.length}</button><button id="cache-hits" aria-pressed="${hits}">字节命中率</button><button id="cache-refresh">刷新对照</button></div><p class="cache-explanation">${hits?'命中率来自P3官方结果，按字节计算；它不等于开启Cache后的加速比。':`加速比 = 声明同计划、同核数的无 Cache 周期 ÷ 有 Cache 周期。本站核对原件哈希、场景与核数；官方结果不含方案哈希，未独立证明评估时实际使用该方案。当前 ${valid.length-paired.length} 个有效方案尚缺配对；主表成绩保持不变。`}</p>${rows.length?`<div class="cache-table-scroll"><table><thead><tr><th>算例</th><th>核数</th>${hits?'':'<th>无 Cache 周期</th>'}<th>有 Cache 周期</th>${hits?'':'<th>加速比</th>'}<th>字节命中率</th><th>来源</th></tr></thead><tbody>${rows.map(c=>{const r=c.best,m=r.metrics;return `<tr><th>${c.case_id}</th><td>${c.cores}</td>${hits?'':`<td>${fmt(Number((m.makespan_cycles*m.cache_gain).toFixed(6)))}</td>`}<td>${fmt(m.makespan_cycles)}</td>${hits?'':`<td>${fmt(m.cache_gain,'cache_gain')}×</td>`}<td>${fmt(m.cache_hit_rate,'cache_hit_rate')}</td><td><a href="/api/v1/records/${esc(r.id)}" target="_blank">原件 ↗</a></td></tr>`;}).join('')}</tbody></table></div>`:'<div class="cache-empty">当前筛选没有可展示的配对数据。可以查看字节命中率，或返回成绩表；已有官方成绩不会因此失效。</div>'}`;
...
function pair(r){if(r.problem!=='P3')return '';if(!r.cache_pair_verified)return '<h4>Cache 收益</h4><p>缺少同计划、同核数配对原件，暂不计算收益。</p>';let withCache=r.metrics.makespan_cycles,noCache=withCache*r.metrics.cache_gain,max=Math.max(withCache,noCache);return `<h4>Cache 配对对照 · ${fmt(r.metrics.cache_gain,'cache_gain')}×</h4><div class="pair"><span>无 Cache</span><i style="width:${130*noCache/max}px"></i><em>${fmt(noCache)}</em></div><div class="pair"><span>有 Cache</span><i style="width:${130*withCache/max}px"></i><em>${fmt(withCache)}</em></div><p>方案哈希由提交者声明；本站未独立复跑配对评估。</p>`;}
