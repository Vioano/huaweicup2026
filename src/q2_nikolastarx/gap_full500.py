"""Fresh 100x1-5 P2 adaptive-gap batch; preflight never scores.
...
stops the batch. Only 500 accepted cells form a full same-algorithm result.
"""
...
COORDS = tuple((f'{case:03d}', cores) for case in range(1, 101) for cores in range(1, 6))
AUXILIARY = ('src/q2_nikolastarx/gap_solver_pilot.py',
             'src/q2_nikolastarx/gap_full500.py')
LIMITS = {'solver': 500, 'E2_api': 1000, 'E0_fallback_reserved': 1000,
          'E0_independent': 500, 'workers': 1, 'solver_seconds': 60,
          'e0_seconds': 60, 'batch_seconds': 7200, 'rss_bytes': 4 << 30,
          'retries': 0}
...
def preflight(manifest, e2_root, runner_commit=None):
    ...
    if (doc['solver_commit'] != SOLVER or doc['baseline_commit'] != BASE
            ...
            or doc['limits'] != LIMITS or len(doc['rows']) != len(COORDS)
            or sorted(doc['auxiliary_sources']) != sorted(AUXILIARY)):
    ...
    if runner_commit:
        ...
        for name in AUXILIARY:
            if (ROOT / name).read_bytes() != git_bytes(runner_commit, name):
                raise ValueError('Auxiliary source differs from runner commit: ' + name)
    ...
    actual = sorted(path.relative_to(ROOT).as_posix()
                    ...
                    if path.relative_to(ROOT).as_posix() not in AUXILIARY)
    if actual != paths:
    ...
    for name, expected in doc['auxiliary_sources'].items():
        if digest((ROOT / name).read_bytes()) != expected:
            raise ValueError('Auxiliary source drift: ' + name)
    ...
    feed_by_cell = {(r['case_id'], r['cores']): r for r in feed}
    if len(feed_by_cell) != 500:
        raise ValueError('Frozen baseline feed lacks 500 unique cells')
    for row, (case, cores) in zip(doc['rows'], COORDS):
        ...
        old = feed_by_cell.get((case, cores))
...
def inspect_solver(case_dir, row, receipt, doc):
    ...
    if (calls['E2_api_attempted'] != len(attempts)
            or calls['native_returns'] != len(attempts) or len(attempts) > 2
            or calls['E0_fallback'] or ledger['possible_E0_fallback_calls']
            or any(not isinstance(a, dict) or a.get('status') != 'native'
                   or not isinstance(a.get('record'), dict)
                   or a['record'].get('route') != 'native'
                   or a['record'].get('status') != 'ok'
                   or a['record'].get('problem') != 2
                   for a in attempts)):
    ...
    expected_sources.update({Path(name).name: sha
                             for name, sha in doc['auxiliary_sources'].items()})
    ...
    if (not isinstance(detail, dict) or detail.get('score_evidence') == 'unknown'
        ...
        raise ValueError('Online score evidence unavailable')
    ...
    old_truth = pinned(row['baseline_truth'])
    if attempts:
        first = attempts[0]
        if (first['plan_sha256'] != canonical(old_plan)
                or first['record'].get('route') != 'native'
                or not same_metrics(first['record'], old_truth)):
            raise ValueError('First native baseline differs from frozen baseline')
    ...
    if matches:
        ...
        if (list(plan['node_to_subgraph'].items()) !=
                list(old_plan['node_to_subgraph'].items())
        ...
        comparison = {'kind': 'frozen_baseline_zero_score',
                      'record': old_truth, 'plan_canonical_sha256': key}
...
def run(doc, manifest, e2_root, output, runner_commit, started):
    ...
    summary = {'status': 'running', 'runner_commit': runner_commit,
               ...
               'calls': {'solver_started': 0, 'E2_api_attempted': 0,
                         'native_returns': 0, 'unknown_in_flight': 0,
                         'E0_fallback_confirmed': 0, 'E0_fallback_possible': 0,
                         'E0_independent_started': 0},
               'call_count_complete': True,
               'scope': ('one frozen solver on 100x1-5; only 500 accepted '
                         'cells permit a full-batch result')}
    save(output / 'summary.json', summary)
    for row in doc['rows']:
        if time.perf_counter() >= deadline:
            summary['status'] = 'stopped_batch_deadline_before_next_cell'
        ...
        try:
            ...
            if ledger_path.exists():
                ...
                summary['calls']['native_returns'] += observed['calls']['native_returns']
                summary['calls']['unknown_in_flight'] += int(observed['request_in_flight'])
            ...
            if time.perf_counter() >= deadline:
                raise TimeoutError('Batch deadline reached before independent E0')
            ...
            old = pinned(row['baseline_truth'])
            entry['old_new'] = {
                'old_makespan': old['makespan'], 'new_makespan': result['makespan'],
                'old_added_ddr_bytes': old['data_movement_bytes']['added_copy_bytes'],
                'new_added_ddr_bytes': result['data_movement_bytes']['added_copy_bytes'],
                'outer_solver_wall_seconds': solver_receipt['wall_seconds'],
                'outer_e0_wall_seconds': e0_receipt['wall_seconds']}
    ...
    summary['accepted_cells'] = len(summary['rows'])
    save(output / 'summary.json', summary)
...
def main():
    ...
    if args.mode == 'preflight':
        print(json.dumps({'status': 'preflight_ok', 'cells': len(COORDS), 'calls': 0,
