...
def summarize(batches, solver_commit):
    ...
    for batch in batches:
        ...
        if spec['methods'][0]['variant'] != 'tensor_packet':
            raise ValueError('this full-coverage report is for tensor_packet only')
    ...
    return {'created_at': datetime.now(timezone.utc).isoformat(), 'solver_commit': solver_commit,
            'source_hashes': source_hashes, 'problem': 'P2', 'algorithm': 'tensor_packet',
...
def main():
    ...
    report = summarize([p.resolve() for p in args.batch], args.solver_commit)
