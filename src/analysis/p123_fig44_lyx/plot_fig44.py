...
def plot_figure(rows: list[dict], summary: list[dict], out_path: Path) -> None:
    ...
    fig, (left, right) = plt.subplots(
        1, 2, figsize=(11.6, 5.8), gridspec_kw={"width_ratios": [1.05, 1.35]}
    )
    fig.suptitle("Figure 4-4 | P1 multicore speedup", fontsize=14, fontweight="bold", y=0.99)
    ...
    for x, y in zip(cores, means):
        left.annotate(f"{y:.2f}", (x, y), xytext=(0, 8), textcoords="offset points", ha="center", color=PALETTE["blue"], fontsize=9)
    ...
    right.text(
        0.0,
        -0.20,
        "Points are all 100 cases per core count; box whiskers use 1.5 IQR.\n"
        "The k=1 plot anchor is fixed at 1.0; its observed ratio is retained in the table.",
        transform=right.transAxes,
        fontsize=8,
        va="top",
        color="#4d4d4d",
    )
    fig.tight_layout(rect=(0, 0.03, 1, 0.95), pad=1.4)
...
def main() -> int:
    ...
    write_csv(
        metrics_path,
        rows,
        ["case", "cores", "baseline_cycles", "makespan_cycles", "observed_speedup", "plot_speedup", "attempt_id", "revision"],
    )
    write_csv(
        summary_path,
        summary,
        ["cores", "n", "mean_speedup", "sd_speedup", "median_speedup", "q1_speedup", "q3_speedup", "min_speedup", "max_speedup", "mean_observed_speedup"],
    )
...
- [ ] Independent desktop-reader inspection of SVG/PDF: not performed in this session.
- [ ] Final paper-width typography acceptance: pending yuanzhifang30-sudo review.
"""
    ...
    manifest = {
        ...
        "limitations": [
            ...
            "SVG/PDF were generated but not independently opened in a desktop reader.",
...
"""
    ...
    auto_review = {
        ...
        "human_review": {
            "required": True,
            "desktop_svg_pdf_opened": False,
            "paper_width_typography_accepted": False,
        },
        "notes": [
    ...
    # Manifest includes the README only after its contents are finalized.
