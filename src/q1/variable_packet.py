...
Uses this repository's original-ID recognizer and frozen official Task adapter.

Adapted from the archived Pro R3 variable_packet.py, source archive commit
29a7b37f83c76c14799ca85a30b8c92d477212c5, with local qualified review.
CLI consumers must require exit status zero and complete diagnostics; a failed
diagnostics write after plan fsync is not a successful delivery.
"""
...
from src.q1.response_compile import (_official_modules, _check_original_graph,
                                     compile_plan, UnsupportedResponse)
from src.q1.response_oracle import simulate, quotient
...
def construct(graph,cores,capacity,bandwidth,gate,compile_limit=512,final_limit=2048):
    ...
    if type(bandwidth) is not int or bandwidth <= 0 or type(gate) is not int or gate < 0:
        raise ValueError('invalid bandwidth or gate')
    if any(type(x) is not int or x < 1 for x in (compile_limit,final_limit)):
        raise ValueError('positive compile budgets required')
