...
The full incident-tensor union is a sufficient no-spill certificate, not a
prediction of official Makespan. No evaluator is used for plan selection.
"""
...
def _resident_by_task(graph: dict, view: dict, capacity: dict) -> tuple[dict, dict] | tuple[None, str]:
    ops = {op["id"]: op for op in graph["ops"]}
    ...
    for task, ids in incident.items():
        if any(sum(tensors[t]["size"] for t in ids if pos[t] == kind) > limit
               for kind, limit in capacity.items()):
            return None, f"original_task_{task}_exceeds_resident_union_capacity"
    return incident, {t: (pos[t], item["size"]) for t, item in tensors.items()}
...
def merge_plan(graph: dict, base: dict, capacity: dict) -> tuple[dict, dict]:
    ...
    info.update(selected="adjacent-union", tasks=len(after["subgraph_ids"]))
    return result, info
...
def construct(graph: dict, cores: int, *, base_constructor=None) -> tuple[dict, dict]:
    ...
    info.update(algorithm_id="q1-adjacent-union-stage-m",
