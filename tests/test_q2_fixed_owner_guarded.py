...
from src.q2_nikolastarx import adaptive_fixed_owner_guarded as route
...
def exercise(values=None, *, old_plans=(BASE, GAP, HYPER), incumbent=HYPER,
    ...
    with patch.object(route.adaptive_hypergap_guarded, 'build', side_effect=old_build), \
         patch.object(route.fixed_owner_reverse, 'retime',
                      return_value=(reverse, {'selected': 'fixed_owner_reverse'}),
                      side_effect=reverse_error) as fixed_owner_build:
        chosen, detail = route.build({}, cores, {}, oracle)
    return chosen, detail, calls, fixed_owner_build


class FixedOwnerGuardTests(unittest.TestCase):
    def test_reverse_strict_improvement_and_old_scores_reused(self):
        chosen, detail, calls, fixed_owner_build = exercise()
        self.assertIs(chosen, REVERSE)
        fixed_owner_build.assert_called_once_with({}, HYPER, {})
        ...
        self.assertEqual(detail['scores'], {'incumbent': [100, 10], 'fixed_owner': [90, 20]})
        self.assertEqual(detail['selected_strategy'], 'fixed_owner_reverse')
    ...
    def test_reverse_score_failure_keeps_old_winner(self):
        ...
        self.assertEqual(calls, [BASE, GAP, HYPER, REVERSE])
        self.assertEqual(detail['skip_reason'], 'fixed_owner_score_unavailable')
        self.assertEqual(detail['oracle_requests'], 4)

    def test_unknown_old_evidence_stops_without_reverse(self):
        chosen, detail, calls, fixed_owner_build = exercise(evidence='unknown')
        self.assertIs(chosen, HYPER)
        self.assertEqual(len(calls), 3)
        self.assertEqual(detail['skip_reason'], 'incumbent_score_evidence_unknown')
        fixed_owner_build.assert_not_called()
    ...
    def test_unsupported_and_unexpected_reverse_failures_keep_incumbent(self):
        for error in (UnsupportedStructure('unsupported'), RuntimeError('failed')):
            ...
            self.assertIs(chosen, HYPER)
            self.assertEqual(len(calls), 3)
            self.assertEqual(detail['skip_reason'], 'fixed_owner_construction_unavailable')

    def test_duplicate_plan_avoids_extra_request(self):
        chosen, detail, calls, _ = exercise(reverse=HYPER)
        self.assertIs(chosen, HYPER)
        self.assertEqual(len(calls), 3)
        self.assertEqual(detail['skip_reason'], 'fixed_owner_duplicates_incumbent')
    ...
    def test_single_core_skips_reverse(self):
        chosen, detail, calls, fixed_owner_build = exercise(cores=1)
        self.assertIs(chosen, HYPER)
        self.assertEqual(len(calls), 3)
