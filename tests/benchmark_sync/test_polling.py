...
class PollingTests(unittest.TestCase):
    def test_backlog_runs_promptly_but_offline_and_rate_limits_still_back_off(self):
        config={'role':'leader','poll_seconds':15}
        ...
        with patch('src.benchmark_sync.__main__.time.time',return_value=100):
            ...
            self.assertEqual(poll_delay(config,{'state':'online','receive':{'pending':0}},0,0),15)
            self.assertEqual(poll_delay(config,dict(active,state='offline'),3,0),120)
            ...
            self.assertEqual(poll_delay(dict(config,role='member'),active,0,0),15)
