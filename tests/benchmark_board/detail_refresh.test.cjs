...
function harness(){
  const pending=[],cells=[1,2].map(i=>({dataset:{p:'P1',case:'00'+i,k:'2'},focus(){document.activeElement=this;}}));
  const document={activeElement:null,querySelectorAll:()=>cells};
  ...
  return {panel,document,cells,pending,get html(){return html;},get renders(){return renders;},get close(){return close;},
