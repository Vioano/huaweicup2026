...
用户确认 P3 的相对加速比也属于核心研发比较项。后续固定算法同时报告并比较有 Cache 的官方 Makespan、同图同核同计划的 `G=M_noL2/M_Cache` 和端到端求解墙钟，另列额外搬运与按字节命中率。实际周期仍按题面优先；不能通过恶化无 L2 的分子或添加重复读取来提高 G。M 与 G 存在取舍时保留证据和不被同时支配的候选，不把命中率或综合单核倍数替代 G。

现有完整 `311322b` 求解器只按 cached Makespan 接受，虽然已有全 500 格同计划配对，**G 尚未进入其候选接受条件**；[实现核查](../../results/a/q3-nikolastarx/forest-cachepair-delta-20260925/OBJECTIVE_AUDIT.md)记录了这一缺口。小型新构造只有 P3 评价时，G 保持未知，不混入旧固定算法全量成绩。本增补不重置历史预算，也不自动批准补齐配对或全量重跑。以下首批六字段与交付记录为历史范围。
