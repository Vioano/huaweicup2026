...
4. **限制条件**：只写 `src/analysis/p1_full_yuanzhifang.py`、本任务卡和 `results/a/p1-full-yuanzhifang-20260924/`。新增 solver/E0/E1/E2 均为 0。原件只读，不改算法/官方配置/中央成绩台，不动其他会话在途进程；不启用 Actions。整批检查失败就保留错误，不部分补零或伪造成功。
...
执行会话：`yuanzhifang30-sudo/s-c909b5a9a43b4bc78807afcf27341a38`。
