...
本地“方案成绩台”的网站维护、成绩数据接收协议、算法来源注册、历史记录、Agent 接口及服务更新，交给专责会话 `nikolastarx/s-7c98eab1093e485291eacb04fd7c59ff`（“制定网站测试数据交接协议”）；原队长调度会话停止这些范围的并行修改，继续全组调度、任务分配、科学复核路由与 PR 整合。LYX、farmer 后续优先支持成绩数据接入、校核和展示更新，由网站维护者给出独立写范围，不再默认承担新的 Coherent 算法研究。当前在途批次先按原授权安全收尾并保存成果，不能因人员调整丢弃历史或自动追加实验。具体接收责任和交接边界见 [团队工作流](docs/TEAM_WORKFLOW.md#方案成绩台与研究调度分工)。
...
用户要求所有 GitHub 协作仅使用免费功能。组织主库和 Vioano 私有镜像的 GitHub Actions 均保持关闭；Git 同步、Issues/team-mailbox、PR 审查与 Atlas 的签名 Git 通道继续使用。不得开启、手动触发或重跑 Actions，不得自行提高消费限额、升级付费计划，或启用会计费的 Codespaces、Copilot 云端任务/审查、LFS/Packages 超额服务及第三方付费 CI。拿不准费用时先查官方规则和账户可用范围，不能通过试跑探测是否收费。
