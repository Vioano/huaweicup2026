...
本方案仅静态设计；没有新门禁实现或控制回归代码，不能据此声称防护已验证。实现须用新的独立文件，提交具体代码 diff 并冻结后交协调审核，再单独批准控制回归。控制回归通过也不自动启动 C/D/E。
...
3. **产物**：本静态握手/句柄生命周期方案、未应用变更说明、四个纯控制用例与明确计数/时间、退出与证据规则。后续实现和执行另审。
...
8. 成员证明和 nonce/阶段/代码身份都成立后，父进程原子发布 ACK，绑定本次真实 PID 和 nonce；子脚本最多再等 5 秒并复核后才进入 payload。纯控制回归的 payload 仅写 `control_payload_reached=true` 后退出；不加载 E2/DLL。未来是否把该握手用于 C/D/E，须另行代码与预算批准。
...
保留原原子 JSON 写、调用前预算、外层 Job、首失败停止机制。此处是设计表，不是已应用 patch。
...
G4 在关闭 Job 前保存完整成员列表并为实际进程打开退出观察句柄，不保留额外 Job 查询句柄。若关闭后 5 秒仍有成员活着，判失败；仅为有界清理可用已知对象名重新打开该同一 Job 的 terminate/query 权限并显式终止，再关闭所有句柄，不能借此重测。记录“因清理重新打开”与“期望 kill-on-close 生效”是两件事。
...
各用例包含启动/门禁/清理最多 20 秒，预留至少 5 秒清理；gate 和 ACK 各最多 5 秒，同时受用例总截止约束。全窗口 300 秒，前45秒完成身份准备，150秒后不启动任何新控制进程；所有计时从新批准后、第一次环境/身份准备之前开始。实际源码/CLI、命令行和参数必须在执行批准前固定，当前不建立窗口或启动用例。
...
提交独立控制实现和新门禁 helper 的固定 diff、完整 import 清单、唯一4个启动入口与无评价入口的审查、handle关闭路径、批准文件和未启动的命令清单。协调对具体代码和300秒/4次启动预算明确批准后，才执行控制回归；此次文档提交不代表该批准。
