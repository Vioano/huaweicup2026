...
**状态：prepared / ready，未扫描原 ZIP，未执行图识别或下面的合成检查，尚无本批分类数据。** 只做了源码读取、AST 对照与语法解析。等待 s59/主控明确安排静态空档，不能因文件已准备好自行运行。当前输出目录没有伪造的 `triage.json`。
...
- 先在同一进程内检查两个极小静态 fixture：长 skip 切口、跨切口外部输入复制、必需 DDR、共享输入交并集、严格拒绝共享张量、零字节 COPY 仍需一周期。没有 Task plan、模拟或评分。这些检查目前仅写好，尚未运行。
...
待生产单次运行后，再据完整输出判断哪些非008图值得下一步固定构造；在此之前不预填084/095的结果、不安排新E0批次。
