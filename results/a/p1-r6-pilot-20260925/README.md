...
Preparation revision 3 is `prepared-manifest-r3.json`. Review found that live-child identity lookup failure could stop without cleanup. The bounded correction uses only the currently held, live and unreaped `Popen(start_new_session=True)` child's ownership to signal its newly created process group; a reaped/historical PID is never sufficient. A fourth real tiny-child test injects getpgid failure and confirms SIGTERM exit with no group residual. All four r3 control tests passed, bringing cumulative tiny children to ten, still zero algorithm/scoring calls. The source/input/scoring budget is otherwise unchanged.
