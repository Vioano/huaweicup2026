...
SESSION='p1-coalesced-e0-008-k5-20260925'
BUNDLE_REMOTE='/content/coalesced-e0-source-bundle.tar.gz'
ARCHIVE_REMOTE='/content/p1_coalesced_e0_window/evidence.tar.gz'
...
def sources(pin):
 ...
 if locked.get('data_head')!='a008dfb8f1b5b881844af312be0b7246b2c6b025' or locked.get('plan_sha256')!='e9327269bc95a81d17ca907a617aed896fd6fdde2a3174044d975f746569f9ce':raise ValueError('artifact manifest identity mismatch')
 ...
 required={'controller.py','controller_draft.py','remote_setup.py','coalesced-e0-source-bundle.tar.gz','bundle-manifest.json'}
 if {r['path'] for r in rows}!=required:raise ValueError('artifact manifest coverage mismatch')
 return {'artifact_manifest_sha256':pin,'data_head':'a008dfb8f1b5b881844af312be0b7246b2c6b025','plan_sha256':'e9327269bc95a81d17ca907a617aed896fd6fdde2a3174044d975f746569f9ce','artifacts_verified':len(rows)}
...
def run_once(*,runner,watchdog_start,verify,now,persist,cli,artifact_manifest_sha256,session=SESSION,archive=None):
 ...
 t0=now();deadline=t0+300;lease=t0+270;log['t0_monotonic']=t0;log['lease_deadline_monotonic']=lease;save()
 ...
 try:
  attempted=True
  r=bounded('new',['new','--session',session],25,260)
  if r.get('returncode')!=0:raise RuntimeError('VM creation unknown/failed')
  r=bounded('upload-bundle',['upload','--session',session,str(HERE/'coalesced-e0-source-bundle.tar.gz'),BUNDLE_REMOTE],15,245)
  if r.get('returncode')!=0:raise RuntimeError('bundle upload failed')
  r=bounded('exec',['exec','--session',session,'--file',str(HERE/'remote_setup.py'),'--timeout','230'],235,35)
  ...
  log['e0_success']=log['evidence'].get('e0_status')=='ok' and log['evidence'].get('E0')==1
