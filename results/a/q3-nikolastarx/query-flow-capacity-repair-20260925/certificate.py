...
def one(case, capacity):
    ...
    for mask in range(1, all_mask + 1):
        ...
        sub = mask
        while sub:
            if sub & anchor and feasible[sub] and 1 + dp[mask ^ sub] < dp[mask]:
    ...
    witness = []
    remaining = all_mask
    while minimum is not None and remaining:
        sub = choice[remaining]
        assert sub and feasible[sub] and sub & remaining == sub
        witness.append({'flows': [i for i in range(r) if sub & (1 << i)],
                        'support_bytes': {pool: sum(ports.tensors[t]['size'] for t in unions[sub][pool])
                                          for pool in capacity}})
        remaining ^= sub
    assert minimum is None or (remaining == 0 and len(witness) == minimum)
    return {'case': case, 'r': r, 'singleton_support_bytes': singleton,
            ...
            'minimum_partition_witness': witness,
            'dp_full_mask': dp[all_mask], 'feasible_mask_sha256': hashlib.sha256(
                bytes(feasible)).hexdigest()}
...
def main():
    ...
    capacity = read_evaluation_config(ROOT / 'data/raw/a/official/data/config.txt')['capacity']
    if capacity != {'L1': 524288, 'UB': 131072}:
        raise ValueError('frozen capacity changed')
    rows = []
    for case in ('005', '086'):
        rows.append(one(case, capacity))
        print(json.dumps({'completed_case': rows[-1]}, ensure_ascii=False), flush=True)
