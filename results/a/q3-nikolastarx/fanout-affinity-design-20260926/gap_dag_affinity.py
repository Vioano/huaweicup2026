# Isolated static prototype. Copied from src/q3/gap_dag.py at SHA-256
# b61b1e33fcddc3491b7c37b12b41f8314d9744909957528296c0684bf57fc219.
# Only import paths and the cuts tie-break/count bookkeeping are changed.
...
def construct(index, cores, bandwidth, cross_delay):
    ...
    # P3 inserts one COPY_OUT/COPY_IN pair per distinct original tensor,
    # producer core and consumer core (unequal), and one per direct op edge.
    # Maintain exact nominal inserted bytes for the committed prefix only.
    owner = {u: j for j, chain in enumerate(chains) for u in chain}
    tensor_size = {t['id']: t['size'] for t in index.graph['tensors']}
    tensor_producers, tensor_consumers, direct_edges = _original_tensor_views(index.graph)
    produced = [dict() for _ in chains]
    consumed = [dict() for _ in chains]
    for tid in tensor_size:
        for u in tensor_producers.get(tid, ()) & index.ops.keys():
            j = owner[u]
            produced[j][tid] = produced[j].get(tid, 0) + 1
        for u in tensor_consumers.get(tid, ()) & index.ops.keys():
            j = owner[u]
            consumed[j][tid] = consumed[j].get(tid, 0) + 1
    touched = [set(produced[j]) | set(consumed[j]) for j in range(len(chains))]
    producer_counts = {tid: [0] * cores for tid in tensor_size}
    consumer_counts = {tid: [0] * cores for tid in tensor_size}
    direct = [(e['source'], e['target'], max(0, int(e.get('data_size', 0))))
              for e in direct_edges if e['source'] in owner and e['target'] in owner]
    direct_touch = [[] for _ in chains]
    for i, (u, v, _) in enumerate(direct):
        direct_touch[owner[u]].append(i)
        if owner[v] != owner[u]:
            direct_touch[owner[v]].append(i)

    def link_count(pc, cc):
        return sum(pc[a] > 0 and cc[b] > 0 for a in range(cores)
                   for b in range(cores) if a != b)

    def incremental_copy_bytes(assignments):
        tentative = dict(assignments)
        affected = set().union(*(touched[j] for j in tentative))
        delta = 0
        for tid in affected:
            before_p, before_c = producer_counts[tid], consumer_counts[tid]
            after_p, after_c = before_p.copy(), before_c.copy()
            for j, c in tentative.items():
                after_p[c] += produced[j].get(tid, 0)
                after_c[c] += consumed[j].get(tid, 0)
            delta += 2 * tensor_size[tid] * (
                link_count(after_p, after_c) - link_count(before_p, before_c))
        direct_indices = set().union(*(direct_touch[j] for j in tentative))
        for i in direct_indices:
            u, v, size = direct[i]
            old_u, old_v = placement.get(owner[u]), placement.get(owner[v])
            new_u = tentative.get(owner[u], old_u)
            new_v = tentative.get(owner[v], old_v)
            before = old_u is not None and old_v is not None and old_u != old_v
            after = new_u is not None and new_v is not None and new_u != new_v
            delta += 2 * size * (after - before)
        return delta

    def commit_copy_counts(j, core):
        for tid, count in produced[j].items():
            producer_counts[tid][core] += count
        for tid, count in consumed[j].items():
            consumer_counts[tid][core] += count
    ...
    while ready:
        ...
        for c in range(cores):
            ...
            if join is None:
                item = ((end, incremental_copy_bytes(((j, c),)), c),
                ...
                for d in range(cores):
                    ...
                    item = ((final, end, incremental_copy_bytes(((j, c), (join, d))), c, d),
                            [(j, c, end, calendar, entries),
