...
冻结静态 `bc003d9` 首次 071 构造在 `unclassified branch joins private flows` 拒绝后停止；069 未运行，0 E0/0 pipe-bound。只读邻接显示有共享原计算源仅向多个私有 O 后处理供参、不向 Q/K/V 投影供参。旧版 S 只取投影共同祖先，漏掉这种源；这是一项通用分解遗漏，不是按节点号修补。

现版先由 row 下游唯一确定各流 O；若一个节点同时属于两流的 row 下游，立即拒绝跨流 join。再从每流的投影、A、O 向上追溯，追溯时只切断已识别的跨流 K/V→A 广播边；共同祖先及其祖先纳入 S。S 若包含任一 A/O/投影则拒绝，确保共享上游不依赖后阶段私有流。余下祖先归 P，未覆盖原计算 op 拒绝。全原边阶段单调、跨流仅真 K/V→A、S 分量整核以及物理支持集检查保持不变。新增 toy：共享 MATMUL 仅向两个 O 供参会被识别为 S 并成功构造；把两个私有 O 再汇入一个下游操作仍被拒绝。`python3 -m unittest discover -s tests -p test_q3_query_flow.py -v` 本修正首轮 3/3 通过（0.002 秒）。未运行真实图、Task/Step/E0/VM；旧失败 run 不回写，新静态 phase 由父会话另行冻结和执行。

审阅后只在 `info` 增加 `stage_counts`（S/P/A/O 的原计算节点数），用于静态分类覆盖核对，不参与候选排序；该纯元数据补充未重复运行完整测试。
