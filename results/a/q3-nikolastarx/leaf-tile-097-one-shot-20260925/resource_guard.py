...
MANIFEST = '7982d1c60fe5df9101535465da429a9311bf4a64a24ee3f45aed3d929db1347b'
...
def main():
    ...
    if not ready:
        ...
        return
    argv = [sys.executable, '-B', '-m', 'src.q3.leaf_tile_probe', 'run', str(OUT),
            '--source', SOURCE, '--forest-root', a.forest_root,
            '--manifest-sha256', MANIFEST,
            '--resource-preflight', str(OUT / 'resource-preflight.json'),
            '--admission-reference', 'coordinator nikolastarx/s-a5bdb19389ee43d686b7976d3bcdf766 2026-09-25T00:05Z single 097/k1 P3 only']
    known = set()
    samples = []
    reason = None
