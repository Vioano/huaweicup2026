"""One-window R9/005/K5 diagnostic supervisor; never constructs a plan."""
import argparse
import hashlib
...
spec = importlib.util.spec_from_file_location('r9_fixed_identity_guard', HELPER)
...
def verify_manifest(m):
    ...
    fixed = {'probe_sha256': ROOT / 'src/q3/layered_query_flow_probe.py',
             'guard_sha256': ROOT / 'src/q3/layered_prepared_guard.py',
             'helper_sha256': HELPER,
             'supervisor_sha256': HERE / 'layered_supervisor.py',
    ...
    if m['inputs']['src/q3/layered_query_flow_probe.py'] != m['probe_sha256'] or (
...
def main():
    ...
    if manifest.get('execution_authorized') is not True:
        raise PermissionError('R9 window has no admission authorization')
    ...
    try:
        ...
        if reason:
            ...
            return
        cmd = [sys.executable, '-B', '-m', 'src.q3.layered_query_flow_probe',
