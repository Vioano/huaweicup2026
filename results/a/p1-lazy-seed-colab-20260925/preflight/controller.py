...
SESSION='p1-lazy-seed-008-20260925'
REMOTE_SHA='b3187697e9aa7e4a16a8122c9cdf34c4bcbd59778d58f4add19715302d4ba143'
...
def sources():
 ...
 if receipt.get('status')!='offline-checked' or receipt.get('source_commit')!='4fc5e5e91e4a2ac9feaf05267c1f8ab713ca5990':raise ValueError('wrong source receipt')
 if sha(HERE/'lazy-seed-source-bundle.tar.gz')!=receipt['bundle_sha256'] or sha(HERE/'bundle-manifest.json')!=receipt['manifest_sha256'] or sha(HERE/'remote_setup.py')!=REMOTE_SHA:raise ValueError('source bundle/manifest/remote SHA mismatch')
...
def run_once(*,runner,watchdog_start,verify,now,persist,cli,session=SESSION,archive=None):
 ...
 try:
  ...
  r=bounded('upload-bundle',['upload','--session',session,str(HERE/'lazy-seed-source-bundle.tar.gz'),BUNDLE_REMOTE],25,200)
