...
BUNDLE=HERE/'period7-source-bundle.tar.gz'
...
def verify_evidence(archive):
 with tarfile.open(archive,'r:gz') as tf:
  ...
  plan=contents.get('workspace/evidence/period7-008/plan.json')
  report=contents.get('workspace/evidence/period7-008/report.json')
...
def plan():
 return {'status':'draft-no-VM-execution','source_bundle_sha256':sha(BUNDLE.read_bytes()),
         ...
         'session':'p1-period7-008-20260925','max_vm_lifetime_seconds':300,
         'steps':['read sessions and require no active session','start independent T0+270s stop watchdog',
         ...
         'limits':{'process_wall_seconds':45,'process_address_space_bytes':536870912,
                   'constructor_wall_seconds':30,'Task_compile':40,'Fraction_response':5,
                   'oracle_queries':4,'period_q':7,'period_s':3,'E0':0,'E1':0,'E2':0,'retry':0},
