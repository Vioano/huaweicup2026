...
try:
 ...
 with tarfile.open(BUNDLE,'r:gz') as tf:
  ...
  assert len(manifest['files'])==len(members)-1 and {r['path'] for r in manifest['files']}==set(names)-{'bundle-manifest.json'}
  for r in manifest['files']:
   p=pathlib.PurePosixPath(r['path']); assert not p.is_absolute() and '..' not in p.parts
   data=tf.extractfile(r['path']).read(); assert len(data)==r['bytes'] and hashlib.sha256(data).hexdigest()==r['sha256']
   dest=WS.joinpath(*p.parts); dest.parent.mkdir(parents=True,exist_ok=True); dest.write_bytes(data)
 (WS/'bundle-manifest.json').write_bytes(manifest_bytes)
 ...
 if not uvpath:
  ...
  if setup['uv_bootstrap']['returncode']: raise RuntimeError('bounded uv bootstrap failed')
  uvpath=shutil.which('uv')
