...
CAPSULE = Path('/content/reverse-generalization-capsule.zip')
ROOT = Path('/content/reverse-generalization-capsule')
OUT = Path('/content/reverse-generalization-results')
RESULTS = Path('/content/reverse-generalization-results.zip')
...
def main():
    expected = os.environ['P2_REVERSE_GENERALIZATION_CAPSULE_SHA256']
    ...
    try:
        ...
        with zipfile.ZipFile(CAPSULE) as z:
            ...
            if (manifest['schema'] != 'reverse-generalization-colab-capsule-v1' or
                    set(names) != set(manifest['files']) | {'manifest.json'} or
                    manifest['limits'] != {'constructor': 5, 'E0': 5, 'E1': 0, 'E2': 0,
        ...
        # The frozen runner owns the 180-second batch and 30-second children.
