"""Two fixed C03 recover attempts from archived C02 plans, no evaluator."""
import argparse
import gzip
import hashlib
...
ROOT = next(p for p in Path(__file__).resolve().parents if (p / "pyproject.toml").is_file() and (p / "src/q2_nikolastarx").is_dir())
sys.path.insert(0, str(ROOT))
from src.q2_nikolastarx import outside_slack_candidate
...
def main():
    ...
    for core in diag['trace_result']['per_core_timeline']:
        for op in core['ops']:
            if op['op_id'] in eligible:
                ...
                finish[op['op_id']] = op['start']
    assert set(finish) == eligible
    adapter = outside_slack_candidate
    c02 = [json.loads((archive / 'c02-real-static-20260925' / f'candidate-{i:02}.json').read_bytes()) for i in range(2)]
    start = time.perf_counter()
    candidates, meta = adapter.rebuild(graph, plan, config, c02, finish,
        incumbent_makespan=diag['score']['makespan'])
    ...
    report = {'source_commit': subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
        ...
        'adapter_calls': 1, 'recover_limit': 2, 'E0': 0, 'E1': 0, 'E2': 0, 'meta': meta,
    ...
    print(json.dumps({'construction_seconds':elapsed, 'meta':meta}))
