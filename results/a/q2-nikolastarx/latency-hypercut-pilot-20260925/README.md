...
One public native E2 request scores each complete candidate. Independent E0 runs only when native Makespan is strictly lower than that cell's old c665 official E0 Makespan. The old E0 is accepted for comparison only after checking graph/config hashes, complete selected plan hash, c665 source set and checkout, E2 source identity, native online ledger, selected canonical plan/score binding, and successful old E0 process. No case-specific algorithm branch or historical score enters construction.
