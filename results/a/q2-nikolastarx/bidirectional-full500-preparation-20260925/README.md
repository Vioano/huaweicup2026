...
The initial 30ab producer verified `source_files` but omitted the `solver_sources` subset consumed by cell receipt validation. Export integration review caught this before any full500 dispatch. The corrected preflight explicitly retains the 67 verified solver Python hashes. The old fixed package was withdrawn with zero solver/E2/E0 calls. A full `cell()` replay now passes for all three existing qualification fixtures using the real preflight, original plans, native ledgers and official results, with subprocess execution replaced by receipt copies in disposable temporary directories. This checks runner plumbing and adds no new algorithm observations. See `cell-replay-receipt.json`; old preflight and supervisor-test receipts remain historical evidence of their actual narrower scope.
