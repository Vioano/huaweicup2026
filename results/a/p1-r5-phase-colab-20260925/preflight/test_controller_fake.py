...
FILES=['controller.py','controller_draft.py','remote_setup.py','r5-source-bundle.tar.gz','bundle-manifest.json','artifact-manifest.json']
...
def parent_crash_watchdog_only_stops_its_frozen_session():
 with tempfile.TemporaryDirectory() as td:
  ...
  launcher=d/'launcher.py';launcher.write_text('''import json,os,pathlib,subprocess,sys\nd,cli,log,deadline,pin=sys.argv[1:]\np=subprocess.Popen([sys.executable,str(pathlib.Path(d)/"controller.py"),"--execute","--watchdog","--cli",cli,"--session","p1-r5-phase-008-k5-20260925","--lease-deadline-monotonic",deadline,"--artifact-manifest-sha256",pin],stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,text=True,start_new_session=True)\nack=json.loads(p.stdout.readline());assert ack["ready"] and ack["session"]=="p1-r5-phase-008-k5-20260925"\npathlib.Path(d,"watchdog.pid").write_text(str(p.pid));os._exit(17)\n''')
  ...
  assert stops==[['--auth','oauth2','stop','--session','p1-r5-phase-008-k5-20260925']]
