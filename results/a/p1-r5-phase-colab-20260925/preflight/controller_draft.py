"""Strict R5 evidence verifier; success means one paired model run, never E0."""
...
def verify_evidence(archive):
 with tarfile.open(archive,'r:gz') as tf:
  ...
  dp='workspace/evidence/r5-008-k5/diagnostics.json'
  cp='workspace/evidence/r5-008-k5/control-plan.json';pp='workspace/evidence/r5-008-k5/phase-plan.json'
  ...
  calls=d.get('calls',{})
  if calls.get('Task_compile_confirmed')!=53 or calls.get('Fraction_attempts')!=2:raise ValueError('Task/Fraction call count mismatch')
  ...
  return {'archive_sha256':sha(archive.read_bytes()),'verified_files':len(rows),'setup_status':setup['status'],'diagnostics_verified':True,'diagnostics_status':d['status'],'graph_sha256':GRAPH,'Task_compile_confirmed':53,'Fraction_attempts':2,'E0':0,'E1':0,'E2':0,'retry':0,'both_traces':True,'same_partition':True}
