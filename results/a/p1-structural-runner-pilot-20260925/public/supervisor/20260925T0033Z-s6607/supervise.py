...
root = Path.cwd()
out = root / 'output/p1-two-cell-supervisor/20260925T0033Z-s6607'
runner = ['python', '-B', 'src/q1_benchmarks/s6607_structural_full500.py', '--output-root', 'results/a/p1-structural-runner-pilot-20260925/20260925T0033Z-s6607', '--cases', '001,051', '--cores', '5', '--workers', '1']
env = os.environ.copy()
...
def snapshot():
    ...
    return rows
proc = subprocess.Popen(['<locked-python-3.12>', '-c', "import subprocess,sys; sys.exit(subprocess.call(sys.argv[1:]) )", *runner], cwd=root, env=env, start_new_session=True, stdout=(out/'runner.stdout.txt').open('xb'), stderr=(out/'runner.stderr.txt').open('xb'))
