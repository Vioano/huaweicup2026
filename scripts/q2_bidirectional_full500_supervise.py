"""Outer, fail-closed Mac supervisor for the fixed local bidirectional full500 producer.
...
SCOPE = 'p2-bidirectional-local-full500'
RSS_LIMIT = 4 << 30
GROWTH_LIMIT = 20 << 30
FREE_RESERVE = 10 << 30
...
def preflight_paths(args):
    ...
    if manifest.get('solver_source_commit') != SOURCE or sha(paths['runner']) != manifest.get('runner_sha256'):
        raise ValueError('runner/source manifest mismatch')
    limits = manifest.get('limits', {})
    if (limits.get('cells') != 500 or limits.get('workers') != 2
    ...
    pins = {'source_commit': SOURCE, 'runner_sha256': sha(paths['runner']),
            'manifest_sha256': sha(paths['manifest']), 'supervisor_sha256': sha(Path(__file__))}
...
def run(args):
    ...
    receipt = {'scope': SCOPE, 'status': 'starting', 'started_at': utc(),
               ...
               'limits': {'seconds': DEADLINE_SECONDS, 'aggregate_rss_bytes': RSS_LIMIT,
                          ...
                          'sample_seconds': SAMPLE_SECONDS, 'workers': 2},
    ...
    try:
        ...
        if (receipt['status'] == 'producer_exited' and receipt['exit_code'] == 0
                ...
                and receipt.get('accepted_cells') == 500
                and receipt.get('accepted_rows') == 500
