"""Dispatch the ten fixed P3 forest shards with a four-cell pilot and four-slot cap.
...
HANDOFF = ROOT / "results/a/q3-nikolastarx/forest500-control-20260925-s59"
AREA = ROOT / "results/a/q3-nikolastarx/forest-full500-20260925-s59"
PILOT = [("058", 5), ("079", 5), ("001", 1), ("082", 4)]
...
def preflight():
    ...
    for n in range(1, 11):
        ...
        if value["solver_commit"] != SOURCE or value["solver_module"] != "src.q3.forest_solve":
            raise RuntimeError("shard source changed")
        if len(value["jobs"]) != 50 or value["budget"] != {
            "max_e0_calls": 150, "total_wall_seconds": 1800, "per_job_seconds": 90
        }:
...
def main():
    ...
    if args.preflight:
        print(json.dumps({"valid": True, "cells": 500, "shards": 10, "max_workers": 4,
    ...
    state = {"schema": "p3-ten-shard-dispatch-v2", "source_commit": SOURCE,
             "control_source_commit": SOURCE,
             "runner_commit": SOURCE, "runner_path": "src/q3/feedback_benchmark.py",
             "controller_path": "scripts/s59ee_p3_forest_dispatch.py", "controller_sha256": sha(__file__),
             "started_at": utc(), "finished_at": None, "status": "running", "max_workers": 4,
             "max_solver_calls": 500, "max_E0_calls": 1500, "deadline_seconds": 1800,
    ...
    while active or queue:
        ...
        if time.monotonic() >= deadline:
            stopped = True
            stop_reason = "global 1800s deadline"
        time.sleep(.5)
