"""Frozen 15d86e1 P2 bidirectional full500 Mac producer; preflight has no score calls."""
...
COORDS = tuple((f'{c:03d}', 5) for c in range(1, 101)) + tuple(
    (f'{c:03d}', k) for k in range(1, 5) for c in range(1, 101))
...
LIMITS = {'cells': 500, 'workers': 2, 'E2_api': 2000,
          ...
          'batch_seconds': 7200, 'rss_bytes_per_cell': 2 << 30,
          'rss_bytes_total': 4 << 30, 'retries': 0, 'E0_fallback_reserve': 2}
...
def preflight(repo, raw_root, e2_root, python, manifest_path):
    global ROOT, PYTHON, E2_ROOT, RAW_ROOT
    ...
    if (doc.get('schema') != 'q2-bidirectional-local-full500-v1'
            or doc.get('solver_source_commit') != SOLVER
            or doc.get('solver_module') != MODULE
            or doc.get('coordinates') != [list(x) for x in COORDS]
            or doc.get('limits', {}).get('workers') not in (1, 2)
            or {k: v for k, v in doc.get('limits', {}).items() if k != 'workers'} !=
