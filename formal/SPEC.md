...
| F-RESOURCE | **8 条规则，均实测** | DDR 等分共享与回溯重算、DDR 端点判定、Cache 只由 COPY_IN 查询且走独立池、FIFO 不晋升 + 超容量不缓存、Cache key 跨 rename 稳定、同刻同时 miss 的幂等 insert、FIFO 淘汰、完成的 COPY_IN 一律 insert（含起飞时命中者在飞被淘汰后重插） |
...
合计 **39 条规则**（37 条 `verified-by-probe`，2 条 `draft-sourced`；后者均为需独立验收的规范条款），
覆盖表见 `coverage.json`。
...
python src/adversarial/verify_l2_r3.py           # 命中在飞被淘汰后完成重插（CACHE-HIT-RETIRE-001）
