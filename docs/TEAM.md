...
**2026-09-25 最新优先级：** 总调度负责跨任务协调、共享资源排程与科学复核路由；`s-7c98…` 转为成绩台被动维护及正式数据接收，原本机生产会话 `s-59ee…` 已完成在途交接，不再派新批。分配、执行、交成绩与网站维护统一先读 [方案成绩台](benchmarks/BENCHMARK_BOARD.md)，具体边界见 [团队工作流](TEAM_WORKFLOW.md#方案成绩台与研究调度分工)。NikolaStarx、Fang 的算法专项负责后续算法与 Coherent 研究；LYX、farmer 持续承担有固定范围的 benchmark 计算。旧算法范围仍是历史及交接责任，不因新分工默认释放单写权。

| 角色 | GitHub 账号 | 职责 |
| --- | --- | --- |
| 队长 | NikolaStarx | A 题首轮：冻结输入、公共接口/E0、独立验收与团队协调 |
| 成绩台接收与维护 | NikolaStarx 的 `s-7c98…` 会话 | 新正式数据、明确请求或可复现故障到达时处理；网站、协议和中央账本单写维护 |
| 本机 benchmark 历史生产 | NikolaStarx 的 `s-59ee…` 会话 | 原批次已交接，不再派新批；既有 runner、原件和结果仍保留供核验 |
| benchmark 计算 | farmeruncle123 | 持续承接输入输出明确的有限批次、原件导出与失败记录；由总调度核容量与写范围，Issue #14。旧 FORM/反例成果与未结验收保留 |
| benchmark 计算与复核 | lyx0217 | 持续承接固定候选、平台复核和明确对照批次；由总调度核容量与写范围，Issue #15。旧 E1 写权及待修事项另行交接，不自动取消 |
