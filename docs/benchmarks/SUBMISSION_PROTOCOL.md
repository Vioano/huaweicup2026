...
维护者反馈接收条数、入榜/待核/失败数和原因，并登记新来源。每个 GitHub actor 将签名 envelope 追加到自己的 `benchmark-submissions/<login>` ref；leader 轮询所有已信任 actor 的 ref，并把签名回执和中央快照写入单独的中央 ref，避免不同电脑竞争写同一个 ref。升级期间 receiver 也读取旧共享 ref 中尚未完成的提交，既有历史不迁写、不覆盖。同步器在线时约每2秒检查，网页每1秒读取已发布快照；**每个完整 feed 独立发布与接收，至少可以只有1条记录，不等 run 结束或达到固定条数，也不为同步补齐500条**。这里的“完整”指该feed自身可解析、原件与哈希齐全，不要求科研run已完成。生产端每形成一条或一小组新完成/修订记录，就导出为新的不可变feed并提交；不反复改写同一路径，也不把已同步的全量前缀重复提交。轮询间隔不是端到端延迟保证；网络、限流、待处理积压、原件下载和接收校验都会增加实际送达时间。未登记来源不进入准入扫描。无需访问队长 localhost 或向网页 POST。旧 `schema_version:1` 数据兼容留存，新包声明 `submission_version:1`，不覆盖旧 feed。
...
`artifacts.<名称> = {"path":"仓库相对路径","sha256":"64位小写SHA256"}`。路径用 `/`，允许 `results/ docs/ data/ tasks/`；不允许绝对路径、反斜线、冒号、`..`、`.git`或指向仓库外的软链接。所有引用须在feed的**同一完整Git提交**中可读；外部原件先保全到本人的授权结果目录。JSON可用 `.json.gz`，SHA256算所存压缩字节，展开后JSON解析上限128MiB；所存单原件（含gzip压缩字节）仍≤64MiB。完整官方轨迹已实测可超过64MiB，保留原字节与哈希，不删减字段；更大原件交维护者核查上限，不自行改写。
