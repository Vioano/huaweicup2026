...
维护者反馈接收条数、入榜/待核/失败数和原因，并登记新来源。双向签名同步器在线时最多每5秒发现并传输已提交的完整 feed，中央接收器每2秒处理inbox，网页每1秒读取新快照；各分片可独立发布，无需等批次结束或凑满500条。离线、限流和大文件会延迟送达，未登记分支不进入固定来源扫描。无需访问队长 localhost 或向网页 POST。旧 `schema_version:1` 数据兼容留存，新包声明 `submission_version:1`，不覆盖旧 feed。
