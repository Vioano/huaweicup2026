...
常规整理在生产端脚本化完成，不转移给接收方；已接收的历史旧格式及适配回执保留。既有结果缺文件时先补现存原件，不为凑字段重跑。双向签名同步在线时，每5秒检查提交与快照，接收器每2秒排空inbox，页面每1秒读变化；已提交的每个完整feed都可独立追加，不等待500条齐备。离线、限流和大文件会延迟实际送达；轮询成功不等于新数据，也不表示另一Agent已读、接手或通过科学验收。
