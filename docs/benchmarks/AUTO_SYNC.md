...
- 每个已提交的完整feed进入持久outbox后即参与下一轮签名传输；新版同步器在线时最多每5秒检查一次远端及本地已提交worktree，成员页面每1秒读取已验签快照，中央接收器仍每2秒排空本地inbox。没有“凑够500条再发送”门槛，分片feed按各自attempt/revision追加，旧历史不会被覆盖。每5秒是轮询间隔而非端到端硬保证：原件体积、验签/准入、网络、限流和离线会增加耗时；状态页分别显示最后成功时间、队列和错误。
