...
数据是双向生产、汇总后共享。只有中央账本决定准入；各端 SQLite 不相互覆盖或合并。成员视图保留全部成功/失败/待核/撤回历史和算法来源；显示“中央已核”，不伪称成员本机重跑或核过每个原件。原件固定 Git 链接按需打开。初始3154条快照约1MB；原件不因压缩快照被删除。
...
提交完成后普通程序自动发现、校验完整字节、保留持久 outbox，必要时将固定提交非强制推到本人的唯一 `benchmark-delivery/<actor>/<id>` 引用，再将签名描述发布到 `benchmark-sync-v1`。其他用户分支和工作区不改。主库已存在的固定提交不重复推送研究历史。也可明确入队：
...
- 内容用 SHA-256，Git blob另校Git对象hash；固定提交、签名代数和全部旧记录分别核查。缺记录、历史改写、回退、相同代数不同内容均拒绝；保留最后可信页面。坏 outbox独立隔离，有效邻居仍处理。
- 每个submission用稳定内容ID，重试和入账后回执丢失不产生第二份历史。`accepted` 只表示中央接收完成，正式eligible/待核/失败另由已有接收器判定，不能把HTTP成功或签名成功当官方算法验证。
- 中央下载先完成各字节/hash，再原子发布 `inbox/<id>/request.json`；只有网站进程同一个 Ledger worker 接收入库。网络缺字节不能误落为永久reported。队长离线则成员排队，不能承诺两台机器离线仍实时。
- 接收器优先处理未有回执的提交，在8秒调度预算内持久轮转；中断的大包下次续传，其他待办仍获得处理机会。已验签的完成项只有提交blob、回执blob及受信公钥集合均不变才跳过重复验签；变化后重新核验。调度缓存损坏只重置调度提示，不改账本或快照。
- 15秒检查是健康空闲时默认轮询间隔，不是传输/校核端到端上限。有变化才写Git，未变化不重下数据。断线退避并遵守GitHub限额/Retry-After；大原件、网络或队长离线会增加延迟，页面显示最后成功时间与错误。
...
`accepted/current.json`是已校验完整快照的原子指针；`status.json`含最后成功时间、数据版本/数量、软件实际版本、队列数量及阶段错误。网站使用 `serve --mirror <current.json> --sync-status <status.json>`；中央使用 `--sync-inbox` 和同状态文件。

本机单测与GitHub读回不是双人Windows验收。最终验收必须分别记录：双方当前UI资源指纹、签名release ID、数据snapshot ID及记录ID集合；双方各一份真实既有benchmark的自动投递/签名回执/两端读回；断网重连、重复投递、UI升级失败回滚和真实Windows自启动。实验成绩不得为了“测试同步”新增或伪造；可以重复投递现有固定feed，预期added=0。
