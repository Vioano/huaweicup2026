...
实现：`src/q1/shared_input_budget.py`。这是独立候选，不与 heavy-suffix 混成 portfolio；固定读取 161cdb35 源线的 bounded/component-pack 机制。当前仅合成契约测试通过，尚无本方法真实图构造或 E0/E1/E2 成绩。
...
每次仅保留当前配置的块/窗口元数据，配置摘要为 O(K²) 级核统计（Task IDs 不枚举为提交计划），图和临时归属字典主要 O(N+E+F)。实际墙钟尚未用真实图测量。正式报告须包含读取输入、这些在线元数据、构造/验证与落盘，外部 E0 单独列。
...
下一轮只建议独立固定的新批次 044/046/090 K5，由父会话或成绩台维护者重新授权预算。每格仅本候选一次构造，再外部 E0；复用旧参考值，保留所有退化/失败。本实现可能得到与初始简化代数表不同的 a/窗口数，原因包括按 Task 计复制、窗口拆分互补 Pipe、逐 COPY 取整及 gates；应以实际 diagnostics 如实报告。
