...
内存状态可以被固定边表示，不等于原图相对 ID 顺序足以确定这些边。官方 `consume_inputs` 和 `release_dead_outputs` 遍历未排序的 Python set；释放信用顺序可能随实际整数 ID 改变。新域是否仍满足旧 `Family.prekey` 的编译等价，需要单独证明或反例，不能由上述容量引理推出。
