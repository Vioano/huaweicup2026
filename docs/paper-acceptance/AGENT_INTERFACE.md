...
| `GET /api/v1/figure-selection` | Fang原18组任务下一内容版的逐图处置：正文9、附录1、合并5、不采用3；监督会话提案已读回，未改冻结v9，也不代表人工验收 |
| `GET /api/v1/figure-requests` | 论文组织任务定义的缺图需求、派工与接收状态；FIG-FANG-5-3记录Fang半成品、验收台修订及v9第46页的同字节入稿，科学和语言仍待审 |
| `GET /api/v1/checkpoints` | 最新冻结稿与当前验收基线的不同版本、发布链接及原件哈希；v9 `figure_inventory` 列出29个实际图号、页码与30份图文件SHA-256，`board_visual_check`只表示验收台抽样看过整页 |
| `GET /checkpoints/v9.pdf`、`GET /checkpoints/v9/46.png` | 本机已注册且逐次核对SHA-256的冻结稿及指定整页预览；旧稿批注坐标不迁移 |
