...
**本地兼容补丁说明**：以下原生 Windows 阻塞结论针对原始 `edf9d83`/上游 0.5.0。经本人明确授权的本轮 Windows 补丁见 [WINDOWS_COMPAT.md](WINDOWS_COMPAT.md)；采用经队长审查的精确补丁提交、通过补丁哈希预检后，可在原生 Windows 另行测试。文件 fsync 保留，但不承诺目录替换的断电耐久性与 POSIX 等价。原版本和补丁版本的结果必须分开记录。
