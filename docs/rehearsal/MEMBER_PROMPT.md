...
我授权你在本轮约 60 分钟内向这个控制 Issue 报到、发送挑战回复和必要测试反馈；在所有 Git 工作区之外建立自己的 Atlas 私有身份和短时同步服务；按队长给本轮分配的任务与权限签名提交请求；在自己的 codex/rehearsal- 分支实际写和运行预演脚本、提交规定结果并发起 PR，不合并。已授权范围内无需每步问我。越权拒绝测试仅按 RUNBOOK 操作本轮独立任务，不能越界修改真实任务、共享权限或他人文件。不共享账号、私钥、令牌、session URL 或完整私有状态。
...
队长和每位队员都必须按 docs/rehearsal/READER_CHECKS.md 完成 U1/U2/R1/R2：本人打开自己的 Atlas 本地网址检查交互；Agent 先自主选择查询策略并回答，再对照评分和跑探针。保留实际问答、同 cursor 的范围、分页完整性、窄查询/全量与增量的调用量和返回字节；宿主不给实际 token 就记未知。不要把接口正常、本人看网页和 Agent 理解混成一个通过结论，不因 probe PASS 代替本人和 Agent 验收。
