...
## v4 增补（F62-R03 新导出回归 + F62-R05 命中率口径，5846099135）

- F62-R03：L1/UB 分面改为四系列互不重叠布局（w=0.18：balanced 实测 x−1.5w、resident 实测 x−0.5w、first_load 实测 x+0.5w、resident 静态 x+1.5w 斜纹），修复 v3 中 resident 静态斜纹盖住 balanced 实测柱的问题；四类数值独立可读（核1 控制 19.2 KiB、核2 控制 420 KiB 等原值全部可见）；648 KiB 三角不变；颜色/斜纹/图例一致。单次运行重导出（rc=0），SVG 仍 468×561.6pt=165.1mm。
- F62-R05：caption 命中率改双口径——按字节 1,622,016/(1,622,016+1,007,840)=61.676989%（约61.7%）；按次数 26/(26+149)=14.857143%（约14.9%）；删除'按次数为 26/175'的混淆表述；balanced/resident 两方案两口径均为 0。未改任何数据。
- 其余文件（cuts.csv/capacity.csv/capacity_detail.csv/plot.py 主体逻辑）不变；图面导出与 audit 哈希按最终字节重算。
