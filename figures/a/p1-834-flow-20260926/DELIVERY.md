...
2026-09-26 论文组织任务读图后要求图面删去内部工作状态。另导出 `p1-834-flow-clean.drawio`、`.svg`、`.png` 与 `print-160mm-clean-preview.png`；图面末行仅保留“官方 E0 独立复评，不进入在线选优”。旧候选文件保持不变，便于对照；`python3 build_flow.py --clean` 生成新版可编辑源和 `semantics-clean.json`，再按下文相同导出选项生成新版图像。论文组织任务仍将独立核查正文位置、图注和科学含义。
