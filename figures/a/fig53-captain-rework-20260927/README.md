...
每个坐标先以同一张图的官方情况 A 单核心完成时间 `B_i` 除以该方法在对应核心数下的官方完成时间 `M_{i,k}`，再对 100 张图的比值作算术平均。共同 `B_i` 取交接包内已核的成员结果表，不使用 `c665` 汇总中表示前一优化版本的 `row.baseline`。`per_case_speedup.csv` 保留真实逐图比值；`method_core_summary.csv` 的 `mean_observed_speedup` 保留实际单核均值。按本图任务书，曲线在一核心处仅显示统一参考起点 `1`，该行的 `display_policy=reference_anchor`，不是改写实际结果。

左侧展示三种全覆盖方法的 1–5 核曲线与 F1 五核心独立点。右侧把同一组五核心数值按方法分行列出，专门区分相差仅约 0.008 的 `tensor` 和 `F1`。图内不放整图标题和长篇解释；适用条件、单核显示规则及方法定义放在 `caption.md`，由 LaTeX 图注和邻近正文排版。图号“5-3”是工作台交付编号，正式稿位置由合稿核定。
...
- `method_core_summary.csv`：16 行，各方法和核心数的样本数、显示均值、真实观测均值和显示规则。
