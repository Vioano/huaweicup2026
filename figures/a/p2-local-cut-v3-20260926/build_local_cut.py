...
nodes = [
    ("scope", "所选两个核心：a、b；至多 16 条可移动计算链\n区域外操作仍在原核心；F 表示同一超边已固定触及的核心集合", 52, 22, 856, 94, "scope"),
    ("a_panel", "", 52, 139, 856, 358, "panel"),
    ("a_title", "例 A｜F = ∅，超边连接两条可移动链 u、v，权重为 w 字节", 76, 157, 808, 43, "heading"),
    ...
    ("b_title", "例 B｜F = {a, c}：已固定触及所选核心 a 和另一个核心 c", 76, 535, 808, 43, "heading"),
    ("b_formula", "a 已在 F 中，不再建立“有链在 a”的容量边；只有新增核心 b 才增加复制。", 78, 588, 804, 37, "formula"),
    ...
    ("guard_panel", "", 52, 808, 856, 366, "panel"),
    ("guard_title", "局部最小割之后，还必须检查计算工作量约束", 76, 828, 808, 44, "heading"),
    ("guard_cut", "求最小割\n精确比较当前区域的\n原始 COPY 字节数", 80, 895, 238, 108, "step"),
    ("guard_cap", "逐核、逐类检查\n计算流水线（Pipe）工作量\n超限：移入链回原核\n并重新求解", 363, 895, 238, 108, "step_small"),
    ("guard_accept", "只在工作量满足上限、\n局部复制字节数严格减少时\n保留新的核心分配", 646, 895, 238, 108, "step"),
    ("guard_bound", "若有 r 条可移动链，最多求解 r + 1 次；原分配始终可行。", 91, 1038, 778, 39, "result"),
    ("guard_limit", "此处只优化原始 COPY 字节数并限制计算工作量；不证明完整方案的完成时间（Makespan）缩短。", 80, 1103, 800, 43, "caveat"),
]
...
palette = {
    ...
    "caveat": ("#FFF6E7", "#B47A37", "#5A3F24", 19, 1),
}
...
semantic = {
    ...
    "symbols": {"a": "chosen source-side core", "b": "chosen sink-side core", "c": "another fixed core",
                "F": "set of cores already touched by pins outside the local region", "w": "positive hyperedge weight in bytes",
    ...
    "verification": {"empty_F": {"offset": "-w", "same_side_cost": "0", "split_cost": "w"},
                     "F_contains_a_and_c": {"offset": "+w", "all_a_cost": "w", "any_b_cost": "2w"}},
}
