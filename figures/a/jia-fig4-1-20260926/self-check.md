...
## v8 修订核对（对应 F41 细化单 5843485201；科学三项已于 5843536714 签通过）

- R02 优先方案执行：layerBottom 标题缩为「下层｜冻结官方评价（提交者不可修改）」，完整脚本路径移至 caption.md/对应表；重导出后 165mm 插入宽度目检：标题包围区与 steps 边框有可见空隙、不接触不遮字，下层箭头端点连接不变。
- R01 复核：f3 style 仅一处 exitY=1，无重复键后写覆盖；导出图线从候选框下缘出发、走行间空隙、箭头落 dedup 上缘，不穿六个候选框或标题。
- R03 复核：E1→方案（f6b 经 x=710 绕诊断框右缘）、单候选→方案（f6 直连）、诊断旁支（f6c 虚线）三条关系清楚，实线不碰任何文字。
- R04 复核：DELIVERY/self-check 为 v8 实际内容；viewBox 710×689 实测自最终 SVG；插入高 160.0mm；字号 10px→6.65pt、11–12px→7.3–8.0pt 按 fontSize×165/宽×72/25.4 换算；无旧值残留。

## v7 修订核对（逐条对应工作台 #14 评论 5843410014，历史保留）
