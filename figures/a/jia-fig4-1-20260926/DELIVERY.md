# 图 4-1 交付包｜P1 两层决策与官方评价流程（v8）

- 图号：4-1　版本：v8（2026-09-26，按 F41-R01~R04 细化单（#14 评论 5843485201）与科学核验补充结论（5843536714：三项技术标准通过、转非阻塞）执行；v7=b9ff50912 及历次稿均保留于 git 历史）
...
2. **下层标题带（F41-R02，v8 按细化单优先方案）**：标题缩为「下层｜冻结官方评价（提交者不可修改）」，完整脚本路径 multicore_cut_evaluate_problem_1.py 由 caption.md 与源码对应表承载；配合 v7 的节点下移 16px/容器 268px，标题文本区与 steps 顶边在 165mm 插入宽度下有可见空隙，不接触、不遮字。
2b. **下层标题带（v7 原做法）**：下层标题（"下层｜官方评价 — 冻结评估器 multicore_cut_evaluate_problem_1.py（提交者不可修改）"）独立留白带加高——read/copy/steps/sim 及两个评价出口整体下移 16px，layerBottom 容器加高至 268px，标题下沿与 steps 顶边（现 y=430）脱离接触；实际 viewBox 710×689（draw.io CLI 导出，含边缘），按 165mm 宽插入高 160.0mm。
